...
func main() {
	...
	default:
		fmt.Println("Invalid option: please provide list or count")
		// The flag.PrintDefaults() does not work here
		flag.PrintDefaults()
	...
	if countCommand.Parsed() {
		...
		// Choice flag
		metricChoices := map[string]bool{"chars": true, "words": true, "substring": true}
		if _, validChoice := metricChoices[*listMetricPtr]; !validChoice {
			countCommand.PrintDefaults()
			os.Exit(1)
		}
